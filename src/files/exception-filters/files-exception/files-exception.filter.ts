...
import { extractFromText } from 'auth/util/regex.util';
...
export class FilesExceptionFilter implements ExceptionFilter {
  ...
  private extractFileTypes(message: string) {
    console.log(message);
    const maxTypesStr = extractFromText(message, this.FILE_TYPES_REGEX);
    const mediaTypesWithBackslashes = maxTypesStr.split('|');
    const mediaTypes = mediaTypesWithBackslashes.map((e) =>
      e.replace('\\', ''),
    );
    const fileTypes = mediaTypes.map((e) => extension(e));
    return fileTypes;
  }
